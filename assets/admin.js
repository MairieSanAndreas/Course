...
async function modaleDecharge(inscription) {
  // Import a la demande : la decharge ne sert qu'a ce bouton. Un import
  // en tete de fichier ferait tomber TOUT le panel si decharge.js
  // manquait — une fonctionnalite annexe ne doit jamais emporter
  // l'application.
  let D;
  try {
    D = await import('./decharge.js');
  } catch (e) {
    notifier("Module de décharge introuvable (assets/decharge.js).", 'erreur');
    return;
  }
  ...
  (async () => {
    ...
    try {
      const blocs = [];
      for (const p of pilotes) {
        const canvas = await D.genererDecharge({ ...p, date });
        ...
        btn.onclick = () => D.telechargerDecharge({ ...p, date });
